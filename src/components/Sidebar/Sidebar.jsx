...
function Sidebar() {
  ...
  return (
    <>
      <div className=' z-10 fixed  scroll-bar2 overflow-y-scroll overflow-x-hidden h-screen' onScroll={(e)=>e.target}>
        <div className={`bg-sidebar sticky h-full  ${open ? "px-4" : "px-3"} pt-8  ${open ? "w-72" : "w-20"} duration-300 ${mobilenav && scrSize < 500 ? "invisible  w-0 " : ""} $ relative  ${mobilenav && 'opacity-0'} `}  onScroll={(e)=>e.target}>
          <ArrowLeftOutlined  className={`bg-primary text-textPrimary text-2xl rounded-full absolute -right-0 top-9 border border-purple-800 cursor-pointer ${!open && "rotate-180"}`} onClick={() => { setOpen(!open) }} />
          ...
          <ul className={`pt-2 `}>
            ...
            <NavLink key={index} to={menu.path}>
                <li title={menu.title} key={index} className={`text-textPrimary text-sm flex items-center gap-x-4 cursor-pointer p-2 rounded-md mt-2 hover:bg-slate-500 ${menu.spacing ? "mt-24" : "mt-1"}`}>
                  ...
                  }} className={`text-base font-medium flex-1 duration-500 ${!open && "hidden"}`}>
                  {menu.title}</span>
                  {menu.submenu && (
                    <DownOutlined className={`${submenuOpen && "rotate-180"} ${!open && "hidden"}`}  />
                  )}
                </li>
                {menu.submenu && submenuOpen && open && (
                  <ul>
                    {menu.submenuItems.map((submenuItem, index) => (
                      ...
                      <li key={index} className={` text-textPrimary ml-4 text-xs flex items-center gap-x-1 cursor-pointer p-1 rounded-md mt-1 hover:bg-slate-500`}>
