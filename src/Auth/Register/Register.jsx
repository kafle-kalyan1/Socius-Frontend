...
// useEffect(() => {
//   showOtpModal({title:"Verify OTP", message:"Enter the OTP sent to your email", icon:"", link:"Didn't get otp",data:{email:"aaa@aa.aa", username:"aaa", password:"aaaa"}, submit:{title:"Verify", action:({data})=>{
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {      
      ...
      axios.post("api/auth/register/", values).then((res) => {
        if (res.status === 201) {
        showLoading(false)
        showOtpModal({title:"Verify OTP", message:"Enter the OTP sent to your email", icon:"", link:"Didn't get otp",data:{email:values.email, username:values.username, password:values.password}, submit:{title:"Verify", action:({data,hide})=>{
          axios.post("api/auth/verify/", data).then((res) => {
            if (res.status === 202) {
              toast.success(res.data.message, {
                duration: 3000,
      ...
      <DynamicLogo/> 
          ...
          <h3 className="text-sm mt-3 font-semibold text-center text-textSecondary dark:text-dark_textSecondary">
            ...
            <p
              ...
              className="font-medium text-secondary hover:underline mt-1 cursor-pointer"
            >
          ...
          <form className="mt-6" onSubmit={formik.handleSubmit}>
            ...
            <Input
              ...
              btn1={{
                icon1: <EyeOutlined />,
                text1: "Hide Password",
                action1: () => setShowPassword(!showPassword),
              }}
              btn2={{
                icon2: <EyeInvisibleOutlined />,
                text2: "Show Password",
                action2: () => setShowPassword(!showPassword),
              }}
            />
            <SelectInput
            formik={formik}
