...
const Message = () => {
  ...
  useEffect(() => {
    ...
    //try to get current user from parameter
      ...
      if (!socket) {
        ...
        newSocket.onmessage = (event) => {
          ...
          setUserList((userList) => {
            ...
            userList[index].last_message = JSON.parse(event.data).message;
            userList[index].timestamp = JSON.parse(event.data).timestamp;
            return [...userList];
          });
        };
      }
  ...
  useEffect(() => {
    // Fetch messages when the component mounts or the currentChatUser changes
    fetchMessages();
  ...
  return (
    <div className="flex h-screen">
      <div className={`block duration-300 z-30 ${open ? "w-1/5" : " w-1/12"}`}>
        <Sidebar className="" />
      </div>
      <div
      ...
      >
        <div className={`flex h-screen`}>
          <div className={`bg-grey-lighter gap-2 flex flex-col overflow-auto ${open ? "w-2/4" : "w-1/4 mr-2"} border-r-2 border-cardBorder z-20 shadow-md ${currentChatUser ? "max-sm:hidden" : open ? " max-sm:w-full max-sm:ml-[40%]" : 
          " max-sm:w-full max-sm:ml-[10%]" } `}>
            {userList.length > 0 && (
              userList.map((user) => (
                <div
                  key={user.username}
          ...
          <div className={`flex flex-col ${open ? "w-3/4" : "w-full"} ${!currentChatUser ? "max-sm:hidden" : open ? "max-sm:block" :"max-sm:block ml-[10%]"} `}>
            ...
            <div className="flex-1 overflow-auto bg-gray-100 p-4 scroll-bar" id="chatContainer">
              ...
              {messages.map((msg, index) => (
                <div key={index} className={`mb-2  flex w-full  ${msg?.sender?.username == currentChatUser ? "justify-start" :
                  "justify-end"
                  } `}>
                  <div
